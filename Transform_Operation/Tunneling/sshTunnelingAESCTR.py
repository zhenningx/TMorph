...
class sshTunneling(Transform_Operation):
    ...
    def validate(self):
        ...
        assert len(self.iv) == 16, 'IV/nonce length must be 16 bytes'
    
    def operate(self):
        self.validate()
        if path.exists("update.pcap"): os.remove("update.pcap")
        if path.exists("update1.pcap"): os.remove("update1.pcap")
        ...
        for pkt in self.packets:
            ...
            if c == str(sorted(self.inputSession,key=str)):
                if firstPacket:
                    ...
                    pkt = pp.buildClientPacket(sether,dether,sip, dip, 'DF', 
                        128, 'tcp', port, 22, 'PA', clientWindow, load)
                    # wrpcap("update.pcap",pkt,append=True)
                    #packet 5
                    load = None
                    pkt = pp.buildServerPacket(dether,sether,dip, sip, 'DF', 
