...
#      wrpcap("update.pcap",__packets)
      ...
      for ul in updateList:
          ...
          for pkt in pkts:
              ...
              if str(sorted(inputSession,key=str)).split(',')[0:4] == self.sortPackets(pkt).split(',')[0:4]:
                  if i > ul[0]:
                      if pkt['IP'].src == ul[2]:
                          pkt.seq = pkt.seq + ul[1]
                      else:
                          pkt.ack = pkt.ack + ul[1]
              i += 1
  ...
  def buildClientPacket(self, sether, dether, sip, dip, ipFlags, ttl, proto,
      ...
      """
      from Fuzz.Transform_Operation.Tunneling.sshTunnelingAES256CTR import sshTunneling
